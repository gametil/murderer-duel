...
local function refreshFriends()
	...
	if ok then friendIds = new end
end
task.spawn(function() task.wait(5); refreshFriends() end)
...
RunS.RenderStepped:Connect(function()
	local ok, err = pcall(function()
		...
		if locked and lockedTarget and lockedTarget.Parent then
			...
			if h and h.Health > 0 and (hrp.Position - lockedTarget.Position).Magnitude <= Range then
				-- stay locked
			else locked = false; lockedTarget = nil end
		end
		...
		if not locked then
			...
			if best then lockedTarget = best; lockedDist = bdist; locked = true end
		end
	...
	if not ok and err then warn("MDUEL:", err) end
end)
